...
import { Camera, Clock, Eye, User, Snowflake, ChevronDown, ChevronUp } from 'lucide-react';
import { TransformedPost } from '@/lib/wordpress';

interface ThroughTheLensProps {
  articles: TransformedPost[];
}

export const ThroughTheLens: React.FC<ThroughTheLensProps> = ({ articles }) => {
    ...
    <section className="space-y-8">
      ...
      {displayedArticles.length > 0 ? (
        ...
        <div className="text-center py-16">
          ...
          <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">
            No Photography Content Available
          </h3>
          <p className="text-gray-600 dark:text-gray-300">
            Check back soon for photography guides and tips.
          </p>
        </div>
      )}
      ...
      <div className="text-center">
        {articles.length > 3 && (
          <>
            ...
            >
              <span>{showAll ? 'Show Less' : 'View All Photography Content'}</span>
              {showAll ? (
